...
//            setLocale(Locale(userPreferences.getAppLanguage().name))
    ...
    private fun hideBottomMenu() {
        navController.addOnDestinationChangedListener { _, destination, _ ->
            when(destination.id) {
                R.id.navigation_search, R.id.navigation_tickets, R.id.navigation_contacts, R.id.navigation_user,
                R.id.nav_driver_home, R.id.driverScheduleFragment, R.id.navigation_journey_fragment, R.id.loginUserFragment,
                R.id.registrationFragment, R.id.passwordRecoveryFragment-> showMenu()
                else -> hideMenu()
            }
        }
    }
